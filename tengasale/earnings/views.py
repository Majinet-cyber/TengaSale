...
def earnings_unlock(request):
    ...
    if request.method == "POST":
        ...
        if profile.earnings_failed_attempt_count == 5:
            audit(request, "EARNINGS_LOCKOUT_TRIGGERED")
...
def earnings_lock_enable(request):
    ...
    if not owner_verification_valid(request, "enable"):
        messages.error(request, "Verify the account owner before enabling Earnings Lock.")
...
def earnings_lock_disable(request):
    ...
    if not ok and not owner_verification_valid(request, "disable"):
        audit(request, "EARNINGS_UNLOCK_FAILED")
...
def earnings_recovery_start(request):
    challenge = start_owner_challenge(request, request.POST.get("purpose", "reset"))
    if challenge:
        ...
        messages.error(request, "A code could not be sent. Wait one minute or contact Tenga Support.")
    return redirect("profile_settings")
...
def earnings_recovery_verify(request):
    ...
    if challenge:
        messages.success(request, "Owner verified. Complete the Earnings security action below.")
    else:
