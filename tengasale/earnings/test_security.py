...
class EarningsLockSecurityTests(TestCase):
    ...
    def test_reset_requires_owner_verification_not_account_password(self):
        self.enable()
        new_pin = "246813"
    ...
    def test_unlock_grant_expires_server_side(self):
        self.enable()
        self.client.post(reverse("earnings_unlock"), {"pin": self.pin})
        session = self.client.session
        grant = session["earnings_unlocked"]
        grant["at"] = (timezone.now() - timedelta(minutes=16)).isoformat()
        session["earnings_unlocked"] = grant
        session.save()
        self.assertRedirects(self.client.get(reverse("earnings_home")), reverse("earnings_unlock"), fetch_redirect_response=False)

    def test_role_permission_denies_earnings_before_pin_policy(self):
        self.profile.earnings_access = False
        self.profile.save(update_fields=["earnings_access"])
        self.assertEqual(self.client.get(reverse("earnings_home")).status_code, 403)
